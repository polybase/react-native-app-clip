...
export const withParentEntitlements: ConfigPlugin<{
  ...
  return withEntitlementsPlist(config, (config) => {
    ...
    console.log(`[withParentEntitlements] config = ${JSON.stringify(config, null, 2)}`)
    ...
    console.log(`[withParentEntitlements] After update, main config = ${JSON.stringify(config, null, 2)}`)
